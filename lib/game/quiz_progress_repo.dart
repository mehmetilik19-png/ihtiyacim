...
class QuizProgressRepo {
  ...
  // season / kalıcı ilerleme
  static const _kSeasonCorrect = 'qp_season_correct';
  static const _kQuestionCursor = 'qp_question_cursor';
  ...
  Future<void> init({required int seed}) async {
    ...
    if (!inited) {
      ...
      await _sp.setInt(_kSeasonCorrect, 0);
      await _sp.setInt(_kQuestionCursor, 0);
      await resetRun();
    }
  }
  ...
  // season
  Future<int> getSeasonCorrect() async => _sp.getInt(_kSeasonCorrect) ?? 0;
  Future<void> setSeasonCorrect(int value) async =>
      _sp.setInt(_kSeasonCorrect, value);

  Future<int> getQuestionCursor() async => _sp.getInt(_kQuestionCursor) ?? 0;
  Future<void> setQuestionCursor(int value) async =>
      _sp.setInt(_kQuestionCursor, value);
